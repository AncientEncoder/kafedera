...
int  main(int argc,char *argv[])
{
    ...
    if(!rank){
        data=loadData(&K,&D,&N);  
        if(size==1||size>N||N%(size-1))    MPI_Abort(MPI_COMM_WORLD,1);  
    }
    MPI_Bcast(&K,1,MPI_INT,0,MPI_COMM_WORLD);  
    MPI_Bcast(&N,1,MPI_INT,0,MPI_COMM_WORLD);
    MPI_Bcast(&D,1,MPI_INT,0,MPI_COMM_WORLD);
    if(rank)    data=array(N/(size-1),D);  
    all_in_cluster=(int *)malloc(N/(size-1)*size*sizeof(int));  
    local_in_cluster=(int *)malloc(N/(size-1)*sizeof(int));  
    in_cluster=(int *)malloc(N*sizeof(int));  
    sum_diff=(float *)malloc(K*sizeof(float));  
    ...
    for(i=0;i<K;i++)    MPI_Bcast(cluster_center[i],D,MPI_FLOAT,0,MPI_COMM_WORLD);  
    if(rank){
        cluster(N/(size-1),K,D,data,cluster_center,local_in_cluster);  
    ...
    do{   
        temp1=temp2;
        if(!rank)    getCenter(K,D,N,in_cluster,data,cluster_center);  
        for(i=0;i<K;i++)    MPI_Bcast(cluster_center[i],D,MPI_FLOAT,0,MPI_COMM_WORLD);  
        if(rank){
            cluster(N/(size-1),K,D,data,cluster_center,local_in_cluster);  
            for(i=0;i<K;i++)    sum_diff[i]=0.0;
